"""
Dataset-Level Evaluation of Latency Encoding Fidelity (Low-Dose)
-----------------------------------------------------------------
Evaluates encoding error on noisy validation images (realistic SNN input).

Purpose:
- Verify encoding behavior on actual SNN input distribution
- Confirm histogram shift doesn't create pathologies
- Validate safety margin in realistic conditions

Metrics:
- Mean PSNR ± std
- Mean SSIM ± std
- Comparison to full-dose encoding
...
def main():
    print("=" * 70)
    print("LOW-DOSE ENCODING EVALUATION (Realistic SNN Input)")
    print("=" * 70)
    print(f"Timesteps: {T}")
    ...
    # Use LOW-DOSE validation slices (realistic SNN input)
    val_slices = low_dose[VAL_SPLIT_START:]
    ...
    # Evaluate each slice
    print("\nEvaluating encoding fidelity on low-dose images...")
    ...
    # Results
    print("\n" + "=" * 70)
    print("RESULTS: LOW-DOSE VALIDATION SET")
    print("=" * 70)
    ...
    print(f"\n📊 Low-Dose Intensity Statistics:")
    print(f"   Mean intensity: {all_intensity.mean():.3f}")
    print(f"   Std intensity:  {all_intensity.std():.3f}")
    print(f"   Min intensity:  {all_intensity.min():.3f}")
    print(f"   Max intensity:  {all_intensity.max():.3f}")
    ...
    axes[0].plot(bin_centers, bin_means, linewidth=2, marker='o', color='orange')
    ...
    axes[0].set_title(f'Low-Dose Encoding Error vs Intensity (T={T})')
    axes[0].legend()
    axes[0].grid(alpha=0.3)
    
    # Plot 2: Intensity Histogram
    axes[1].bar(bin_centers, bin_counts, width=0.04, alpha=0.7, 
                edgecolor='black', color='orange')
    axes[1].set_xlabel('Intensity')
    axes[1].set_ylabel('Pixel Count')
    axes[1].set_title('Low-Dose Intensity Distribution\n(Note: Shifted toward darker values)')
    axes[1].grid(alpha=0.3, axis='y')
    
    plt.tight_layout()
    plt.savefig('encoding_low_dose_analysis.png', dpi=150, bbox_inches='tight')
    print("✓ Saved: encoding_low_dose_analysis.png")
    
    # PSNR distribution
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.hist(psnr_values, bins=20, edgecolor='black', alpha=0.7, color='orange')
    ax.axvline(psnr_values.mean(), color='r', linestyle='--', 
               label=f'Mean: {psnr_values.mean():.2f} dB')
    ax.set_xlabel('PSNR (dB)')
    ax.set_ylabel('Number of Slices')
    ax.set_title('Low-Dose Encoding PSNR Distribution')
    ax.legend()
    ax.grid(alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig('encoding_low_dose_psnr_dist.png', dpi=150, bbox_inches='tight')
    print("✓ Saved: encoding_low_dose_psnr_dist.png")
    
    print("\n" + "=" * 70)
    print("✓ EVALUATION COMPLETE")
    print("=" * 70)
    print("\nKey Observation:")
    print("  If encoding MSE << CNN MSE on low-dose images,")
    print("  then T=32 is validated for SNN implementation.")
